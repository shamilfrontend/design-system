...
const update = (): void => {
  ...
  cursorTop.value = ((100 - qPickerDropdown.hsvaModel.value) * height) / 100;
};
...
const handleDrag = (event: MouseEvent): void => {
  ...
  emit('change', { ...qPickerDropdown.hsvaModel, saturation, value });
};

watch(
  () => [qPickerDropdown.hsvaModel.saturation, qPickerDropdown.hsvaModel.value],
  async () => {
    await nextTick();
    update();
  },
