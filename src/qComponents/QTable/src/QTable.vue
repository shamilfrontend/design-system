...
const emit = defineEmits<{
  'update:checkedRows': [value: number[]];
  'update:sortBy': [value: QTablePropSortBy];
  'update:groupsOfColumns': [value: QTablePropGroupsOfColumns];
}>();
...
const isRowClickable = computed<boolean>(() =>
  Boolean(!props.isLoading && attrs.onRowClick)
);
...
const updateGroupsOfColumns = (value: QTablePropGroupsOfColumns): void => {
  emit('update:groupsOfColumns', value);
};

const emitRowClick = (row: Row, rowIndex: number): void => {
  const { onRowClick } = attrs;
