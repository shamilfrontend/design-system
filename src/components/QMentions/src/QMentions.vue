...
const placeholderText = computed(
  () => props.placeholder ?? t('QMentions.placeholder')
);
...
function selectOption(option: QMentionOption): void {
  ...
  const idx = val.lastIndexOf(props.prefix);
  const next =
    idx === -1 ? val + option.label : val.slice(0, idx) + option.label + ' ';
...
<template>
  <div class="q-mentions">
    ...
    <ul
      ...
      class="q-mentions__dropdown"
    >
      <li
        v-for="opt in filtered"
        :key="opt.value"
        class="q-mentions__option"
