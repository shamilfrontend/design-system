...
import { Q_DROPDOWN_INJECTION_KEY, type QDropdownContext } from './constants';
import type {
...
function handleDocumentClick(event: MouseEvent): void {
  ...
  if (reference.value?.contains(target) || dropdown.value?.contains(target)) {
    return;
  }
...
onMounted(() => {
  ...
  switch (props.trigger) {
    ...
    case 'contextmenu':
      reference.value.addEventListener('contextmenu', handleContextMenu, false);
...
onUnmounted(() => {
  ...
  reference.value?.removeEventListener('contextmenu', handleContextMenu, false);
